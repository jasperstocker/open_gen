...
namespace opengen.types
{
	...
	public class DelaunayPoint
	{
		...
		{
			int shapeSize = AdjacentTriangles.Count;
			Vector2[] shape = new Vector2[shapeSize];
			int tIndex = 0;
			foreach (DelaunayTriangle delaunayTriangle in AdjacentTriangles)
			{
				shape[tIndex] = delaunayTriangle.circumcenter.vector2;
				tIndex++;
			}
			return shape;
		}
		...
		{
			return new DelaunayPoint(Numbers.RoundToInt(Numbers.Lerp(a.x, b.x, t)),
				Numbers.RoundToInt(Numbers.Lerp(a.y, b.y, t)));
		}
		...
		public static DelaunayPoint Rotate(DelaunayPoint input, float degrees)
		{
			...
			DelaunayPoint output = new DelaunayPoint(Numbers.RoundToInt((cos * tx) - (sin * ty)),
				Numbers.RoundToInt((sin * tx) + (cos * ty)));
			return output;
		}
